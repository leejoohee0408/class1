...
//				String query = "insert into todo values(seq_todo.nextval, '"+todoDTO.getTodo()+"',sysdate, null, 'N')";
	...
	// 여기는 수정한거 업데이트 하는곳
	int updatesku(P_skuDTO p_skuDTO) {
		 System.out.println("p_skuDTO updatesku 실행");
		 int result = -1;
			...
			try {
				...
				// [SQL 준비]
				// GPT를 보고 고친것
				String query = "UPDATE p_sku SET ";
				query += "sku_code = ?, sku_name = ?, sku_size = ?, ";
				query += "vendor_name = ?, price = ?, modify_date = sysdate, category = ? ";
				query += "WHERE sku_id = ?";
				...
				ps.setString(6, p_skuDTO.getCategory());
				//GPT를 보고 고친것
				ps.setInt(6, p_skuDTO.getSku_id());
